...
============================================================
  ...
  [1] 🟢 샘플 쿼리 확인 및 백엔드 필터 PR 올리기
      담당자  : 준호
      마감일  : 금요일 오전
      신뢰도  : high
      근거    : "제가 오늘 안에 샘플 쿼리를 확인해보겠습니다. 금요일 오전까지 백엔드 필터 PR을 올리겠습니다."

  [2] 🟢 목요일 오후까지 프론트 화면 붙이기
      담당자  : 서연
      마감일  : 목요일 오후
      신뢰도  : high
...
------------------------------------------------------------
  ...
  [3] 검색 로그
      이유    : 개인정보 이슈로 인해 이번 베타에서는 제외
      근거    : "저는 이번 베타에서는 로그를 안 넣고, 다음 스프린트에서 익명화 설계를 먼저 하는 게 맞다고 봅니다."

  [4] 금액 범위 검색
      이유    : 이번 베타에서 제외 결정
      근거    : "금액 범위 검색은 이번 베타에서 제외합니다. 정확히 일치만 해요."

  [5] 자연어 날짜 검색
      이유    : 이번에는 하지 않기로 결정
      근거    : "아니요. 이번에는 자연어 날짜 검색은 하지 않습니다."
...
============================================================
  ...
  ✅ [VERIFIED] 샘플 쿼리 확인 및 백엔드 필터 PR 올리기
  ...
  ✅ [VERIFIED] 목요일 오후까지 프론트 화면 붙이기
      ✓ evidence_quote_존재: 원문에서 앞뒤 구절 모두 확인됨
      ✓ deadline_원문_확인: 원문에서 키워드 확인됨: ['목요일', '오후']
...
- 초기 구현에서 LLM이 간혹 `confidence` 필드에 "high/medium/low" 이외의 값(예: "높음")을 반환하는 경우 발견 → `_validate_schema()`에서 허용 값 외 입력 시 `low`로 강제하도록 처리
- `evidence_quote`가 빈 문자열 `""`로 반환되는 경우 None 체크만으로는 걸러지지 않는 것 확인 → `not validated_item["evidence_quote"]`로 수정
